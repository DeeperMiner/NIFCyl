...
def parse_arguments():
    ...
    parser.add_argument('--data_set', type=str, default='Tunnel', choices=['Tunnel'])
...
def train(data_list, log_dir, log_name, ckpt_dir, id=None):
    ...
    for shape_idx, shape_name in enumerate(train_set.cur_sets):
        ...
        for iter_i in range(1, args.max_iter+1):
            ...
            pcl_raw_sub = data['pcl_raw_sub'].to(_device) if 'pcl_raw_sub' in data else None     
...
def test(data_list):
    ...
    for shape_idx, shape_name in enumerate(test_set.cur_sets):
        ...
        pcl_raw, nor_gt = test_set.get_data(shape_name)         
        start_time = time.time()
        ...
        if num_point <= max_n:
            ...
            pcl_source = torch.from_numpy(pcl_new).float()           
            grad_norm = np.zeros((pcl_new.shape[0], 3))              
            with torch.no_grad():
                for i in range(k):
